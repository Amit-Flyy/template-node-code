...
import { GLOBAL_SALT } from "../configs.js";
...
myUtil.generateHashWithGlobalSalt = async (password) => {
    if (!GLOBAL_SALT) console.log('NO GLOBAL SALT PRESENT');
    return bcrypt.hash(password, GLOBAL_SALT)
}

myUtil.generateHashWithUseAuthSalt = async (password) => {
    if (!GLOBAL_SALT) console.log('NO GLOBAL SALT PRESENT');
    return bcrypt.hash(password, GLOBAL_SALT)
}
